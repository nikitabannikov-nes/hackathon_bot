...
@router.callback_query((F.data == "add_schedule") | (F.data == "edit_schedule"))
async def add_schedule(call: CallbackQuery, state: FSMContext):
    await call.message.delete()

    print((call.data == "edit_schedule"), F.data, "edit_schedule")
...
async def process_schedule_file(message: Message, state: FSMContext):
    ...
    await process_schedule_excel(message.bot, data['is_edit'], file_info)
...
async def process_schedule_excel(bot, is_edit, file_info: dict):
    try:
        ...
        print(is_edit, parse_schedule_excel(df))

        result = (is_edit, parse_schedule_excel(df))
