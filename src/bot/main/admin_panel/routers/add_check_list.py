...
from admin_panel.admin_kb import admin_panel_kb

router = Router()

# Состояния для загрузки чек-листа
...
async def process_checklist_file(bot, file_info: dict):
    try:
        ...
        df = pd.read_excel(
            ...
            engine='openpyxl',
            header=None  # ← ВАЖНО: нет заголовков
        )
...
def get_non_empty_values(df: pd.DataFrame) -> list:
    ...
    for i in range(len(df)):
        value = df.iat[i, 0]  # используем iat для прямого доступа к значениям
